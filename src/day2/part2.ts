...
for (const line of lines) {
  ...
  const altSequences: number[][] = [];
  for (let i = 0; i < nums.length; i++) {
    altSequences.push([...nums.slice(0, i), ...nums.slice(i + 1)])
  }
  ...
  if (validDiff(nums) && (isDecreasing(nums) || isIncreasing(nums))) {
    ...
    for (const seq of altSequences) {
      if (validDiff(seq) && (isDecreasing(seq) || isIncreasing(seq))) {
        safe++;
        break;
      }
    }
...
console.log('safe sequences:', safe); // 324